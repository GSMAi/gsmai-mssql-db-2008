...
CREATE PROCEDURE [dbo].[aggregate_currency_based_metric]
...
DECLARE @is_decimal bit, @weighting_is_decimal bit, @currency_id int, @is_spot bit
...
CREATE TABLE #data (id bigint, organisation_id int, metric_id int, attribute_id int, date datetime, date_type char(1) COLLATE DATABASE_DEFAULT, value decimal(22,8), currency_id int, weighting_value decimal(22,8), connections_value decimal(22,8))
...
INSERT INTO #data
SELECT	ds.id, ds.organisation_id, ds.metric_id, ds.attribute_id, ds.date, ds.date_type, CASE @is_decimal WHEN 1 THEN ds.val_d ELSE ds.val_i END, ds.currency_id, null, null
...
FROM	#data d INNER JOIN ds_organisation_data ds ON (d.organisation_id = ds.organisation_id AND d.date = ds.date AND d.date_type = ds.date_type) INNER JOIN currency_rates cr ON (ds.currency_id = cr.from_currency_id)
...
BEGIN
	...
	SELECT	null,
			...
			d.date,
			d.date_type,
			SUM(d.value * cr.value * d.weighting_value / d2.weighting_sum),
			@currency_id,
			@is_spot,
	...
	FROM	#data d INNER JOIN
			...
			currency_rates cr ON (cr.from_currency_id = d.currency_id AND cr.to_currency_id = @currency_id) INNER JOIN
			(
	...
	FROM	#data d INNER JOIN
			...
			(
				...
				FROM	#data d INNER JOIN organisation_zone_link oz ON d.organisation_id = oz.organisation_id INNER JOIN zone_link zl ON oz.zone_id = zl.subzone_id INNER JOIN zone_link zl2 ON zl.zone_id = zl2.subzone_id INNER JOIN zones r ON zl2.zone_id = r.id
	...
	FROM	#data d INNER JOIN
			...
			(
				...
				FROM	#data d INNER JOIN organisation_zone_link oz ON d.organisation_id = oz.organisation_id INNER JOIN zone_link zl ON oz.zone_id = zl.subzone_id INNER JOIN zones s ON zl.zone_id = s.id
	...
	FROM	#data d INNER JOIN
			...
			currency_rates cr ON (cr.from_currency_id = d.currency_id AND cr.to_currency_id = @currency_id) INNER JOIN
			(
				SELECT	c.id zone_id, SUM(d.weighting_value) weighting_sum, d.date, d.date_type
				FROM	#data d INNER JOIN organisation_zone_link oz ON d.organisation_id = oz.organisation_id INNER JOIN zones c ON oz.zone_id = c.id
				GROUP BY c.id, d.date, d.date_type
			) d2 ON (c.id = d2.zone_id AND d.date = d2.date AND d.date_type = d2.date_type)
...
SELECT 'Finished: aggregate_currency_based_metric (1m 13s)'
