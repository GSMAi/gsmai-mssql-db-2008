...
CREATE PROCEDURE [dbo].[calculate_absolute_from_percentage_from_zone_data]
...
WHERE	ds.metric_id = @metric_id AND ds.attribute_id = @attribute_id AND ds.date >= @date_start AND ds.date < @date_end AND ds.date_type IN ('Q','H','Y') AND is_calculated = 0
...
SELECT 'Finished: calculate_absolute_from_zone_percentage (2s)'
